...
| 4 | Three assets buckets | `make buckets` creates `$ASSETS_BUCKET_PREFIX-us-east-2`, `-us-west-2` and `-us-east-1` (the observer stack reads its template from the bucket in its own region) with public access blocked. |
| 5 | Third-party images in **your** ECR | `make mirror` copies the digest-pinned images in `src/mirror/images.json` (Argo CD, Valkey as its cache, dex, metrics-server) into `<account>.dkr.ecr.<region>.amazonaws.com/eks-mr-demo/mirror/*` in both regions. **Not optional**: the node subnets have no NAT and no route to the internet, so nothing can be pulled from a public registry. The identity running it needs ECR create/push in both regions -- a policy scoped to `cdk-*` repositories denies the mirror while every other step stays green. |
