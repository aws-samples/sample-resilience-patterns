...
# An ABSENT stack owns nothing: the CLI exits 254 ("Stack ... does not exist") and,
# under set -e + pipefail, that status would abort the whole script at this
# assignment -- which is how a pre-flight on a CLEAN account died (e2e run on
# e51c90c; every earlier run had a region stack present here). Absent -> 0 -> the
# idempotent sweep below is a no-op.
for r in "$PRIMARY" "$SECONDARY"; do
  owned=$({ aws cloudformation list-stack-resources --region "$r" --stack-name "$PROJECT-region-$r" \
    ...
    --output text 2>/dev/null || true; } | wc -w)
  [ "$owned" -eq 0 ] || continue
