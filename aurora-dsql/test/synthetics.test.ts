...
describe('SyntheticsStack', () => {
  ...
  test('creates 2 canaries (local + cross-region for Aurora)', () => {
  ...
  test('creates 2 alarms (one per canary)', () => {
    template.resourceCountIs('AWS::CloudWatch::Alarm', 2);
  });

  test('alarms check SuccessPercent < 100', () => {
    template.hasResourceProperties('AWS::CloudWatch::Alarm', { Threshold: 100, ComparisonOperator: 'LessThanThreshold' });
  });
});
